...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tableRows.forEach((row, index) => {
        row.addEventListener('click', function() {
            const nameRating = this.querySelector('#NameRating').textContent;
            const descRating = this.querySelector('#DescRating').textContent;

            const editInput = document.getElementById('EditCategory');
            const inputContainer = document.querySelector('.input-container');
            const descInput = document.getElementById('DescCategory');

            editInput.value = nameRating;
            descInput.value = descRating;
